...
"""
Lane Navigation using PID and Lane Detection using waypoints 
"""
...
class VehiclePIDController():
	...
	def __init__(self, vehicle, args_lateral, args_longitudinal, max_throttle = 0.75, max_break = 0.3, max_steering = 0.6):
	...
	def run_step(self, target_speed, waypoint):
		...
		accleration = self.long_controller.run_step(target_speed)
		current_steering = self.lat_controller.run_step(waypoint)
		control = carla.VehicleControl()
		
		if accleration >=0.0:
			control.throttle = min(abs(accleration), self.max_break)
			control.brake = 0.0
		else :
			control.throttle = 0.0
			control.brake = min(abs(accleration), self.max_break)
...
class PIDLongitudinalControl():
	...
	def __init__(self, vehicle, K_P=1.0, K_D=0.05, K_I=0.0, dt=0.03):
...
class PIDLateralControl():
	...
	def pid_controller(self, waypoint, vehicle_transform):
		...
		v_end = v_begin + carla.Location(x = math.cos(math.radians(vehicle_transform.rotation.yaw)), y = math.sin(math.radians(math.radians(vehicle_transform.rotation.yaw))))
...
def main():
	...
	try:
		...
		spawnpoint = carla.Transform(carla.Location(x=-75.4,y=-1.0,z=15), carla.Rotation(pitch = 0, yaw = 180, roll = 0))# Points using trail and error methods
		vehicle = world.spawn_actor(vehicle_bp, spawnpoint)
		actor_list.append(vehicle)
		
		control_vehicle = VehiclePIDController(vehicle, args_lateral={'K_P':1,'K_D':0.05,'K_I':0.0}, args_longitudinal={'K_P':1,'K_D':0.05, 'K_I':0.0})
